...
pub async fn evaluate(
    ...
    let has_active_meds_nudge = existing_nudges.iter().any(|n| {
        n.nudge_type == "meds_not_logged" && (n.state == "active" || n.state == "snoozed")
    });
        ...
        let nudge_id = storage
            .insert_nudge(NudgeInsert {
                ...
                inference_snapshot_json: Some(
                    serde_json::json!({ "meds_pending": true, "source": "context" }).to_string(),
                ),
    ...
    let has_prep_nudge = existing_nudges.iter().any(|n| {
        n.nudge_type == "meeting_prep_window" && (n.state == "active" || n.state == "snoozed")
    });
    if prep_window_active && !has_prep_nudge {
        ...
        let nudge_id = storage
            .insert_nudge(NudgeInsert {
                ...
                signals_snapshot_json: None,
                inference_snapshot_json: Some(
                    serde_json::json!({ "prep_window_active": true, "source": "context" })
                        .to_string(),
                ),
                metadata_json: Some(explanation),
            })
    ...
    let has_commute_nudge = existing_nudges.iter().any(|n| {
        n.nudge_type == "commute_leave_time" && (n.state == "active" || n.state == "snoozed")
    });
    if commute_cfg
        ...
        if let Some((lvl, msg)) = level_message {
            ...
            let nudge_id = storage
                .insert_nudge(NudgeInsert {
                    ...
                    signals_snapshot_json: None,
                    inference_snapshot_json: Some(
                        serde_json::json!({ "leave_by": leave_by, "source": "context" })
                            .to_string(),
                    ),
                    metadata_json: Some(explanation),
                })
    ...
    if morning_drift && !has_drift_nudge {
        ...
        let nudge_id = storage
            .insert_nudge(NudgeInsert {
                ...
                signals_snapshot_json: None,
                inference_snapshot_json: Some(
                    serde_json::json!({ "morning_drift": true, "source": "context" }).to_string(),
                ),
                metadata_json: Some(explanation),
            })
            .await?;
