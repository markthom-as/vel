...
pub async fn run(storage: &Storage) -> Result<usize, crate::errors::AppError> {
    ...
    let travel_minutes = first_event
        .and_then(|e| e.payload_json.get("travel_minutes").and_then(|p| p.as_i64()))
        .unwrap_or(0);
    ...
    let next_commitment_id = open_commitments
        .first()
    ...
    let (attention_state, drift_type, drift_severity, attention_confidence, attention_reasons): (
        ...
        (
            ...
            Some("high"),
            0.75,
            vec!["prep window active", "prep dependency unresolved", "no progress signal"],
        )
        ...
        (
            ...
            0.7,
            vec!["morning not started", "meds commitment open", "no workstation signal"],
        )
    } else if morning_started && prep_window_active {
        ("aligned", None, None, 0.8, vec!["morning underway", "prep window active"])
    } else if morning_started {
    ...
    if material {
        if let Err(e) = storage.insert_context_timeline(now_ts, &context_str, None).await {
    ...
    if let Err(e) = storage
        .emit_event("STATE_CHANGED", "inferred_state", None, &serde_json::json!({ "state_name": state_name }).to_string())
        .await
    {
...
fn is_material_context_change(prev_json: Option<&str>, new_json: &str) -> bool {
    ...
    let Ok(prev_val) = serde_json::from_str::<serde_json::Value>(prev) else { return true };
    let Ok(new_val) = serde_json::from_str::<serde_json::Value>(new_json) else { return false };
    for key in [
        "morning_state",
        "mode",
        "next_commitment_id",
...
mod tests {
    ...
    fn material_change_identical_returns_false() {
        let ctx = r#"{"morning_state":"underway","mode":"morning_mode","prep_window_active":false}"#;
