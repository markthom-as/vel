...
fn known_profile_ids(
    ...
    submitted_openai_profiles: Option<&[OpenAiCompatProfileUpdateRequest]>,
    ...
    if let Some(profiles) = submitted_openai_profiles {
...
pub fn load_llm_settings() -> Result<LlmSettingsData, AppError> {
    ...
    let mut profiles = profile_sources
        .into_iter()
        .map(|source| LlmProfileSettingsData {
...
pub fn apply_llm_settings_update(
    ...
    if let Some(openai_profiles) = request.openai_compat_profiles.as_ref() {
        ...
        for profile in openai_profiles {
            ...
            write_profile_file(&target_path, profile)?;
    ...
    let known_ids =
        known_profile_ids(&existing_sources, request.openai_compat_profiles.as_deref())?;
...
mod tests {
    ...
    #[test]
    fn apply_llm_settings_update_writes_profiles_and_routing() {
        ...
        std::env::set_var("VEL_MODELS_DIR", &dir);
        let settings = apply_llm_settings_update(&LlmSettingsUpdateRequest {
