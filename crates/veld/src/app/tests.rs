use super::*;
use crate::middleware::{OPERATOR_AUTH_HEADER, WORKER_AUTH_HEADER};
...
use vel_api_types::{
    ProjectCreateRequestData, ProjectFamilyData, ProjectProvisionRequestData, ProjectRootRefData,
};
use vel_core::{
...
async fn work_assignment_lifecycle_claims_updates_and_lists_receipts() {
    ...
    let list_bytes = axum::body::to_bytes(list_response.into_body(), usize::MAX)
        .await
        .unwrap();
    let list_payload: vel_api_types::ApiResponse<Vec<vel_api_types::WorkAssignmentReceiptData>> =
...
async fn claim_next_work_skips_fresh_failed_retry_until_backoff_expires() {
    ...
    crate::services::client_sync::update_work_assignment_receipt(
        &state,
        vel_storage::WorkAssignmentUpdate {
            receipt_id: claimed.receipt_id.clone(),
            status: vel_core::WorkAssignmentStatus::Failed,
            started_at: None,
...
async fn canonical_day_explain_references_commitments_and_signals() {
    ...
    assert!(
        commitment_ids.contains(&meds_id.as_str()) || commitment_ids.contains(&prep_id.as_str()),
    ...
    let meds_nudges: Vec<_> = json["data"]
        ...
        .filter(|n| {
            n["nudge_type"].as_str() == Some("meds_not_logged")
                && (n["state"].as_str() == Some("active") || n["state"].as_str() == Some("snoozed"))
        })
        .collect();
    assert!(
...
async fn accepting_commute_buffer_suggestion_updates_overrides_and_recomputes_context() {
    ...
    assert_eq!(
        accept_json["data"]["adaptive_policy"]["active_override"]["source_suggestion_id"].as_str(),
    ...
    let nudges_after_body = axum::body::to_bytes(nudges_after.into_body(), usize::MAX)
        .await
        .unwrap();
    let nudges_after_json: serde_json::Value = serde_json::from_slice(&nudges_after_body).unwrap();
    assert!(
    ...
    let patch_resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"web_settings":{"dense_rows":false,"reduced_motion":true}}"#.to_string(),
                ))
                .unwrap(),
        )
        .await
...
async fn integrations_store_sensitive_tokens_separately_from_public_settings() {
    ...
    assert!(
        google_public.get("client_secret").is_none() || google_public["client_secret"].is_null()
    );
    assert!(
        google_public.get("refresh_token").is_none() || google_public["refresh_token"].is_null()
    );
...
async fn integrations_logs_endpoint_lists_recent_sync_history() {
    ...
    crate::services::integrations::record_sync_success(&storage, "notes", 2)
        .await
        .unwrap();
    crate::services::integrations::record_sync_error(&storage, "notes", "notes snapshot missing")
        .await
        .unwrap();
    let app = build_app(
        storage,
    ...
    storage
        .set_current_context(
            ...
            &current_context_json(CurrentContextV1 {
                ...
                attention_reasons: vec!["recent git activity indicates active work".to_string()],
...
async fn now_task_lane_patch_assigns_next_up_commitment_to_current_day() {
    ...
    let timezone = crate::services::timezone::ResolvedTimeZone::parse("America/Denver").unwrap();
    let due_at = updated
        .due_at
