...
use uuid::Uuid;
use vel_api_types::{ApiResponse, ContextTimelineEntry, CurrentContextData, EndOfDayData, MorningData, TodayData};
...
pub async fn current(State(state): State<AppState>) -> Result<Json<ApiResponse<Option<CurrentContextData>>>, AppError> {
    ...
    let data = row.map(|(computed_at, context_str)| {
        ...
        CurrentContextData { computed_at, context }
    });
    let request_id = format!("req_{}", Uuid::new_v4().simple());
...
pub async fn timeline(
    ...
    let entries: Vec<ContextTimelineEntry> = rows
        ...
        .filter_map(|(id, timestamp, context_json)| {
            ...
            Some(ContextTimelineEntry { id, timestamp, context })
        })
        .collect();
    let request_id = format!("req_{}", Uuid::new_v4().simple());
