...
fn task_record() -> CanonicalObjectRecord {
    ...
    CanonicalObjectRecord {
        id: "task_01phase61blackbox".to_string(),
        ...
        facets_json: json!({"title":"Black box test","task_type":"generic"}),
    ...
    WorkflowContext {
        ...
        bindings: vec![WorkflowBinding {
            ...
            value: WorkflowContextValue::CanonicalObject(WorkflowObjectRef {
                object_ref: "task_01phase61blackbox".to_string(),
                object_type: "task".to_string(),
...
#[tokio::test]
async fn manual_invocation_black_box_executes_action_and_skill_steps() {
    let pool = pool().await;
...
#[tokio::test]
async fn manual_invocation_black_box_pauses_on_approval() {
    let pool = pool().await;
    let runner = WorkflowRunner::default();
...
#[tokio::test]
async fn manual_invocation_black_box_refused_skill_path_stays_mediated_and_auditable() {
    let pool = pool().await;
    let runner = WorkflowRunner::default();
    let mut modules = BTreeMap::new();
    modules.insert(
    ...
    assert!(runs.iter().any(|record| record.status == "refused"));
    assert!(audits.iter().any(|record| record.status == "denied"));
}

#[tokio::test]
async fn manual_invocation_black_box_dry_run_keeps_runtime_evidence_only() {
    let pool = pool().await;
    let runner = WorkflowRunner::default();
    let mut modules = BTreeMap::new();
    modules.insert(
