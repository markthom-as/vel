...
use crate::todoist_ids::{todoist_integration_account_id, TODOIST_MODULE_ID, TODOIST_PROVIDER};

#[derive(Debug, Clone)]
pub struct TodoistCheckpointState {
    pub sync_cursor: Option<String>,
    pub activity_cursor: Option<String>,
}

#[derive(Debug, Clone)]
pub struct TodoistAccountLinkRequest {
...
pub async fn link_todoist_account(
    pool: &SqlitePool,
    request: &TodoistAccountLinkRequest,
) -> Result<IntegrationAccountRecord, StorageError> {
    let account_id = todoist_integration_account_id(&request.external_account_ref);
...
fn merge_metadata(metadata_json: &JsonValue, checkpoints: &TodoistCheckpointState) -> JsonValue {
    ...
    map.insert(
        "provider".to_string(),
        JsonValue::String(TODOIST_PROVIDER.to_string()),
    );
    map.insert(
        "module_id".to_string(),
        JsonValue::String(TODOIST_MODULE_ID.to_string()),
    );
    map.insert(
...
#[cfg(test)]
mod tests {
    use super::{link_todoist_account, TodoistAccountLinkRequest, TodoistCheckpointState};
    use serde_json::{json, Value as JsonValue};
    ...
    #[tokio::test]
    async fn todoist_account_linking_is_deterministic_and_checkpoint_aware() {
        let pool = test_pool().await;
        let record = link_todoist_account(
            &pool,
            &TodoistAccountLinkRequest {
                external_account_ref: "todoist_primary".to_string(),
                ...
                checkpoints: TodoistCheckpointState {
                    ...
                    activity_cursor: Some("activity_456".to_string()),
                },
            },
        )
        .await
        ...
        assert_eq!(record.provider, "todoist");
        assert_eq!(
