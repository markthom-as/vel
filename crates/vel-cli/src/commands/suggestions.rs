...
pub async fn run_list(client: &ApiClient, state: Option<&str>, json: bool) -> anyhow::Result<()> {
    let resp = client.list_suggestions(state, Some(50)).await.context("list suggestions")?;
    ...
    for s in data {
        println!("{}  {}  {}  {}", s.id, s.suggestion_type, s.state, s.created_at);
    }
    Ok(())
}
...
pub async fn run_inspect(client: &ApiClient, id: &str) -> anyhow::Result<()> {
    ...
    let s = resp.data.as_ref().ok_or_else(|| anyhow::anyhow!("no data"))?;
    println!("id:              {}", s.id);
    println!("suggestion_type: {}", s.suggestion_type);
    println!("state:           {}", s.state);
    println!("created_at:      {}", s.created_at);
    println!("resolved_at:     {:?}", s.resolved_at);
    println!("payload:         {}", serde_json::to_string_pretty(&s.payload)?);
    Ok(())
}

pub async fn run_accept(client: &ApiClient, id: &str) -> anyhow::Result<()> {
    let resp = client.update_suggestion(id, "accepted", None).await.context("accept suggestion")?;
    let s = resp.data.as_ref().ok_or_else(|| anyhow::anyhow!("no data"))?;
...
pub async fn run_reject(client: &ApiClient, id: &str) -> anyhow::Result<()> {
    let resp = client.update_suggestion(id, "rejected", None).await.context("reject suggestion")?;
    let s = resp.data.as_ref().ok_or_else(|| anyhow::anyhow!("no data"))?;
...
pub async fn run_modify(client: &ApiClient, id: &str, payload: Option<&str>) -> anyhow::Result<()> {
    let payload_value = payload
        ...
        .transpose()?;
    let resp = client.update_suggestion(id, "modified", payload_value).await.context("modify suggestion")?;
    let s = resp.data.as_ref().ok_or_else(|| anyhow::anyhow!("no data"))?;
