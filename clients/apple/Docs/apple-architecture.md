...
- overdue path: quick reactions use the backend overdue proposal/confirm/apply contract and fall back to typed confirmation when voice parsing is uncertain
...
- mapping: actions go through existing `VelWatchStore` call paths (`markTopNudgeDone`, `snoozeTopNudge`, `submitThreadText`, `applyOverdueShortcut`) to preserve backend/API contract boundaries
