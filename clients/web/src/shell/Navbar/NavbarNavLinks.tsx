...
export function NavbarNavLinks({
  ...
  const isNudgesActive = isMobile && activeView === 'now' && activeAnchor === 'nudges-section';

  const primaryItems = [...primarySurfaces].map((item) => ({
    key: item.view,
    label: isMobile && item.view === 'system' ? 'Settings' : item.label,
    icon: surfaceIcon(item.view),
    isActive: activeView === item.view && !(item.view === 'now' && isNudgesActive),
  ...
  const nudgeItem = {
    ...
    isActive: isNudgesActive,
    onActivate: () => {
      if (onDeepLink) {
        onDeepLink({ view: 'now', anchor: 'nudges-section' });
        return;
      }
      onSelectView('now');
    },
  ...
  return (
    <nav
      className={cn(
        ...
        isMobile ? 'w-full' : 'items-center gap-x-2 sm:gap-x-3',
      )}
