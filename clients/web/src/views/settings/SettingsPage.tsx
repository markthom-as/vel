...
interface EditableLlmProfileDraft {
  id: string;
  base_url: string;
  model: string;
...
const SETTINGS_SECTIONS: Array<{
  ...
  { key: 'profile', label: 'Profile', tab: 'general' },
  { key: 'clients-sync', label: 'Clients & Sync', tab: 'general' },
  { key: 'integrations', label: 'Integrations', tab: 'integrations' },
  { key: 'configuration', label: 'Configuration', tab: 'runtime' },
  { key: 'backups', label: 'Backups', tab: 'runtime' },
  { key: 'permissions-policies', label: 'Permissions & Policies', tab: 'general' },
  { key: 'templates', label: 'Templates', tab: 'general' },
...
export function SettingsPage({
  ...
  const [defaultChatProfileDraft, setDefaultChatProfileDraft] = useState('');
  const [fallbackChatProfileDraft, setFallbackChatProfileDraft] = useState('');
  const [openAiProfilesDraft, setOpenAiProfilesDraft] = useState<EditableLlmProfileDraft[]>([]);
  ...
  const [llmActionState, setLlmActionState] = useState<ComponentActionState | null>(null);
  ...
  const activeTab = useMemo<SettingsTab>(() => (
    SETTINGS_SECTIONS.find((section) => section.key === activeSection)?.tab ?? 'general'
  ), [activeSection]);
  const setActiveTab = (tab: SettingsTab) => setActiveSection(settingsSectionForTab(tab));
  ...
  useEffect(() => {
    setDefaultChatProfileDraft(settings.llm?.default_chat_profile_id ?? '');
    setFallbackChatProfileDraft(settings.llm?.fallback_chat_profile_id ?? '');
    setOpenAiProfilesDraft(
      (settings.llm?.profiles ?? [])
        ...
        .map((profile) => ({
          ...
          model: profile.model,
          context_window:
            profile.context_window == null ? '' : String(profile.context_window),
  ...
  const daemonRestartPending = Boolean(pendingComponentActions.daemon);
  ...
  const saveLlmSettings = async () => {
    setLlmActionState(null);
    setSaving(true);
    try {
      const response = await updateSettings({
        llm: {
          default_chat_profile_id: defaultChatProfileDraft.trim() || null,
          fallback_chat_profile_id:
            fallbackChatProfileDraft.trim() === ''
              ? null
              : fallbackChatProfileDraft.trim(),
          openai_compat_profiles: openAiProfilesDraft.map((profile) => ({
            id: profile.id.trim(),
            ...
            context_window:
              profile.context_window.trim() === ''
                ? null
                : Number.parseInt(profile.context_window.trim(), 10),
            enabled: profile.enabled,
          })),
        },
      });
      if (!response.ok || !response.data) {
        throw new Error(response.error?.message ?? 'Failed to save LLM settings');
      }
      setQueryData<SettingsData>(settingsKey, () => response.data as SettingsData);
      setLlmActionState({
        status: 'success',
        message: 'LLM settings saved. Restart the daemon to reload routing and profiles.',
      });
    } catch (error) {
      setLlmActionState({
        status: 'error',
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
        <div className="mx-auto max-w-4xl space-y-4">
          <header className="flex items-end justify-between gap-4 border-b border-zinc-900 pb-4">
            ...
            {saving ? <p className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Saving</p> : null}
          </header>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <label className="grid gap-2 text-sm text-zinc-300">
                    ...
                    <input
                      ...
                      onBlur={() => void update('node_display_name', nodeDisplayNameDraft.trim() || null)}
                  ...
                  <label className="grid gap-2 text-sm text-zinc-300">
                    ...
                    <input
                      ...
                      onBlur={() => void update('timezone', timezoneDraft.trim() || null)}
                      className="rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-zinc-100"
                    />
                  </label>
                </div>
            ...
            <>
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Sync transport" value={clusterBootstrap?.sync_transport || 'Unknown'} />
                  <InfoStat label="Continuity" value={`${pendingExecutionHandoffs.length} pending`} />
            ...
            <>
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Freshness" value={settings.backup?.trust?.freshness?.state ?? 'Unknown'} />
              ...
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Guidance" value={`${settings.backup?.trust?.guidance?.length ?? 0}`} />
            ...
            <>
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Linked clients" value={`${linkedNodes.length}`} />
                  <InfoStat label="Pending handoffs" value={`${pendingExecutionHandoffs.length}`} />
                  <InfoStat label="Blockers" value={`${agentInspect?.blockers?.length ?? 0}`} />
                </div>
            ...
            <>
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Constraints" value={`${planningProfile?.planning_constraints.length ?? 0}`} />
                  <InfoStat label="Operator guide" value={onboardingGuide.headline} />
                  <InfoStat label="Current mode" value={agentInspect?.grounding.current_context?.mode ?? 'Not grounded'} />
                </div>
                <p className="mt-4 text-sm text-zinc-400">
            ...
            <>
              <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <InfoStat label="Needs review" value={`${nowData?.review_snapshot?.projects_needing_review ?? 0}`} />
                  <InfoStat label="Action items" value={`${nowData?.action_items?.length ?? 0}`} />
                  <InfoStat label="Open components" value={`${components.length}`} />
                </div>
                <p className="mt-4 text-sm text-zinc-400">
      ...
      <section className="space-y-8">
        ...
        <section>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-900/70 p-4 space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-1">
                <span className="text-xs uppercase tracking-wide text-zinc-500">Default chat profile</span>
                <select
                  value={defaultChatProfileDraft}
                  onChange={(event) => setDefaultChatProfileDraft(event.target.value)}
                ...
                >
                  ...
                  {llmProfiles.map((profile) => (
                    <option key={`llm-default-${profile.id}`} value={profile.id}>
              ...
              <label className="space-y-1">
                <span className="text-xs uppercase tracking-wide text-zinc-500">Fallback chat profile</span>
                <select
                  value={fallbackChatProfileDraft}
                  onChange={(event) => setFallbackChatProfileDraft(event.target.value)}
                  className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-200 focus:border-zinc-500 focus:outline-none"
                >
                  ...
                  {llmProfiles.map((profile) => (
                    <option key={`llm-fallback-${profile.id}`} value={profile.id}>
                      {profile.id} · {profile.provider}
                    </option>
                  ))}
                </select>
            ...
            {fixedLlmProfiles.length > 0 ? (
              <div className="rounded-md border border-zinc-800 bg-zinc-950/60 p-3">
                <p className="text-xs uppercase tracking-wide text-zinc-500">Checked-in profiles</p>
                <ul className="mt-2 space-y-2 text-sm text-zinc-300">
                  {fixedLlmProfiles.map((profile) => (
                    <li key={`llm-fixed-${profile.id}`} className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                      <span>{profile.id} · {profile.model}</span>
                      <span className="text-zinc-500">{profile.base_url}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="space-y-3">
              ...
              {openAiProfilesDraft.map((profile, index) => (
                <div key={`llm-openai-${index}`} className="rounded-md border border-zinc-800 bg-zinc-950/60 p-3">
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <label className="space-y-1">
                      <span className="text-xs uppercase tracking-wide text-zinc-500">Profile id</span>
                      <input
                        type="text"
                        value={profile.id}
                        onChange={(event) => setOpenAiProfilesDraft((current) => current.map((entry, entryIndex) => (
                    ...
                    <label className="space-y-1">
                      <span className="text-xs uppercase tracking-wide text-zinc-500">Base URL</span>
                      <input
                        type="text"
                        value={profile.base_url}
                        onChange={(event) => setOpenAiProfilesDraft((current) => current.map((entry, entryIndex) => (
                          entryIndex === index ? { ...entry, base_url: event.target.value } : entry
                        )))}
                        className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-200 focus:border-zinc-500 focus:outline-none"
                      />
                    </label>
                    ...
                    <label className="space-y-1">
                      <span className="text-xs uppercase tracking-wide text-zinc-500">Context window</span>
                      <input
                        type="number"
                        min="1"
                        step="1"
                        ...
                        onChange={(event) => setOpenAiProfilesDraft((current) => current.map((entry, entryIndex) => (
                          entryIndex === index ? { ...entry, context_window: event.target.value } : entry
                        )))}
                        className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-200 focus:border-zinc-500 focus:outline-none"
                      />
                    </label>
        ...
        <section>
          <div className="mb-3">
            <h3 className="text-lg font-medium text-zinc-200">Daemon control</h3>
            <p className="text-sm text-zinc-500">
              Restart the daemon from Settings after changing runtime-owned config like LLM routing.
            </p>
          </div>
