...
.PHONY: build build-api build-web clean dev dev-api dev-web download-chat-model check-llm-setup check-apple-swift apple-open apple-setup-simulator apple-build-ios-sim apple-build-watch-sim apple-run-ios-sim apple-build apple-run apple-build-ios-device apple-install-ios-device apple-build-watch-device apple-install-watch-device apple-list-devices apple-test install-web lint-web seed smoke test test-api test-web verify verify-repo-truth ci fmt-check clippy-check bootstrap-demo-data container-build container-up container-down container-config docker-build docker-up docker-down podman-build podman-up podman-down nix-shell-info nix-dev-api nix-build
