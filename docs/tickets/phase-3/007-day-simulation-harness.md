...
- **Trace Verification**: Capture run events or trace records alongside state snapshots so replay can assert workflow completeness, not just end state.
...
- Replay can assert that key workflow boundaries emitted the expected events or traces.
