...
- **Now operator surface:** the main web shell now has a `Now` view backed by a dedicated read-only `GET /v1/now` contract, so the page renders one coherent snapshot for summary, schedule, tasks, attention, freshness, and debug state instead of stitching together four endpoints client-side.
