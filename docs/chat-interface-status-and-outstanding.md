...
| 036     | **Partial** | Markdown blocks, inline code/links/emphasis, and fenced code blocks with copy affordance are implemented for text/system messages; richer feedback/action rows remain open |
...
- **Ticket pack:** [docs/tickets/](tickets/) — 001–037 tracked; 036 is partial, 037 is implemented.
...
- **Settings / integrations operator surface:** the Settings page now includes `/api/integrations` management for Google Calendar and Todoist plus recent-run retry/block controls, with frontend tests covering inline action isolation and websocket-aware run state updates. Integration cards also surface typed recovery guidance when credentials are missing, sync is stale, or the last sync failed.
- **Loop control surface:** the Settings page now also exposes `/v1/loops` for runtime loop enable/disable and interval control, instead of leaving that operator flow CLI-only.
- **Local adapter control plane:** `/api/integrations` now also reports local adapter status for activity, git, messaging, notes, and transcripts, while the Settings page can trigger their existing `/v1/sync/*` routes directly.
- **Now operator surface:** the main web shell now has a `Now` view backed by a dedicated read-only `GET /v1/now` contract, so the page renders one coherent snapshot for summary, schedule, tasks, attention, freshness, and debug state instead of stitching together four endpoints client-side. Backend task ordering now prioritizes urgent medication, overdue work, and due-today Todoist items ahead of no-due backlog tasks, the UI flags aging/stale/error/disconnected sources inline instead of letting stale sections masquerade as live data, freshness entries now carry concrete recovery guidance for degraded integrations, both inference plus `Now` timestamp formatting run against the same resolved IANA timezone from operator settings, the schedule section distinguishes “no calendars selected” from “no upcoming events” while surfacing prep/travel/leave-by timing directly on event cards, and the page now refetches on focus plus a background interval while exposing the raw snapshot fields behind a debug drawer.
- **Suggestions operator surface:** the main web shell now includes a dedicated Suggestions view backed by `/v1/suggestions` and `/v1/suggestions/:id`, so operators can inspect pending steering proposals, review attached evidence and decision context, and accept/reject suggestions without dropping to the CLI.
