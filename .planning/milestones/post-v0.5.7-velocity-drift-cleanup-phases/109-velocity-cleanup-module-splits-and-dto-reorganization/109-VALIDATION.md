...
- fourth DTO slice: `integrations` module owns `IntegrationFamilyData` and `IntegrationSourceRefData`, with root re-exports preserving existing consumer imports
...
- fourth DTO slice: `cargo test -p vel-api-types -- --nocapture`, targeted integration connection route test, `cargo check -p vel-cli --all-targets`
