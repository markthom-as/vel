...
## Deferred Follow-On `0.5.7` Requirement Buckets
...
| ADAPTER-57-01 | Platform adapters remain future work and did not ship in `0.5.7`. |
| CALL-57-01 | Duplex thread call mode remains future work and did not ship in `0.5.7`. |
...
- keep duplex voice deferred in `docs/future` until a future milestone explicitly reopens it
- start `0.5.8` at `Phase 01` so active planning stays milestone-local under `.planning/phases/`
